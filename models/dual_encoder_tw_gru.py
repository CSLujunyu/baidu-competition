...
def dual_encoder_tw_gru_model(
    ...
    with tf.variable_scope("encoder-context"):
        ...
        for i in range(config['max_turn_num']):
            context_encoded_outputs, context_encoded = tf.nn.bidirectional_dynamic_rnn(
                ...
                dtype=tf.float32)
            all_turn_encoded.append(tf.concat([context_encoded[0],context_encoded[1]], axis=1))
    ...
    with tf.variable_scope("encoder-candidate"):
        ...
        for i in range(config['batch_size']):
            ...
            temp_outputs, temp_states = tf.nn.bidirectional_dynamic_rnn(
                ...
                dtype=tf.float32)
            all_candidate_encoded.append(tf.concat([temp_states[0],temp_states[1]], axis=1)) # since it's a tuple, use the hidden states
    ...
    with tf.variable_scope("tw"):
        turn_cell = tf.nn.rnn_cell.GRUCell(
            config['rnn_dim']*2
        )
        ...
        reg_tw = tf.contrib.layers.l2_regularizer(config['reg_rate'])(graph.get_tensor_by_name(
            'tw/rnn/gru_cell/candidate/kernel:0')) + tf.contrib.layers.l2_regularizer(
            config['reg_rate'])(graph.get_tensor_by_name('tw/rnn/gru_cell/candidate/kernel:0'))

    with tf.variable_scope("prediction"):

        # Dot product between generated response and actual response
        # c * r logits.shape = (batch_size, options)
        logits = tf.einsum('bk,bjk->bj',turns_states,all_candidate_encoded)
    ...
    return probs, logits, reg_context + reg_candidate + reg_tw
